...
$sql = "SELECT * FROM user, list_student WHERE list_student.id_student = $id_student AND user.id_User = list_student.id_User";
...
if(isset($_POST['submit'])){
	...
	if(isset($firstName) && isset($lastName) && isset($dateOfBirth) && isset($username) && isset($password) && isset($email) && isset($gender) ){
		...
		$sql = "UPDATE user, list_student  SET firstName = '$firstName',lastName ='$lastName', dateOfBirth = '$dateOfBirth', username = '$username', password = '$password', email = '$email', gender = '$gender' WHERE list_student.id_student = $id_student AND user.id_User = list_student.id_User  ";
		$query = mysqli_query($conn, $sql);
...
<a href="admin.php?page_layout=list-stu" class="btn btn-secondary active"
