...
$sql = "SELECT * FROM user, list_teacher WHERE list_teacher.id_teacher = $id_teacher AND user.id_User = list_teacher.id_User";
...
if(isset($_POST['submit'])){
	...
	if(isset($firstName) && isset($lastName) && isset($dateOfBirth) && isset($username) && isset($password) && isset($email) && isset($gender) ){
		...
		$sql = "UPDATE user, list_teacher  SET firstName = '$firstName',lastName ='$lastName', dateOfBirth = '$dateOfBirth', username = '$username', password = '$password', email = '$email', gender = '$gender' WHERE list_teacher.id_teacher = $id_teacher AND user.id_User = list_teacher.id_User  ";
		$query = mysqli_query($conn, $sql);
